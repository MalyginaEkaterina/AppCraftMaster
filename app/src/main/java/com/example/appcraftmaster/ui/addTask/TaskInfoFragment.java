...
public class TaskInfoFragment extends Fragment {
    ...
    public void addTask() {
        if (editTextAddTaskName.getText().toString().isEmpty()) {
            ...
            try {
                Task task = new Task(editTextAddTaskName.getText().toString(), editTextAddTaskDescription.getText().toString(), category.getId());
                Gson gson = new Gson();
                JSONObject offerJson = new JSONObject(gson.toJson(task));
                ...
                JsonObjectRequest request = new Common.JsonObjectRequestWithToken(PreferenceManager.getDefaultSharedPreferences(getContext().getApplicationContext()),
                        ...
                        response -> {
                            ...
                            if (status.getStatus().equals(StatusCode.STATUS_OK)) {
                                ((MyApp) getActivity().getApplicationContext()).setNeedUpdateMyTasks(true);
